...
use crossterm::{
    event::{
        ...
        Event, KeyCode, KeyEvent, KeyboardEnhancementFlags, KeyModifiers, MouseButton, MouseEvent,
...
fn highlight_color(idx: usize, theme: &str) -> Color {
    ...
    match HIGHLIGHT_NAMES.get(idx).copied().unwrap_or("") {
        ...
        "operator" | "punctuation" | "punctuation.bracket"
            | "punctuation.delimiter" | "punctuation.special" => t.operator,
...
fn check_vscode_associations(settings_path: &std::path::Path, file: &std::path::Path) -> Option<&'static str> {
    ...
    for pair in map_str.split(',') {
        ...
        let lang_raw = parts.next()?.trim().trim_matches(|c: char| c == '"' || c.is_whitespace());
...
fn check_gitattributes(gitattrs_path: &std::path::Path, file: &std::path::Path) -> Option<&'static str> {
    ...
    for line in content.lines() {
        ...
        let pattern_name = std::path::Path::new(pattern)
            ...
            .and_then(|n| n.to_str())
            .unwrap_or(pattern);
        if !glob_matches(pattern_name, file_name) { continue; }
...
fn detect_lang_by_name_or_content(fname: &str, path: &str) -> Option<&'static str> {
    ...
    for line in head.iter() {
        ...
        if let Some(pos) = l.find("vim:") {
            ...
            for part in after.split_whitespace().chain(after.split(':')) {
                ...
                if let Some(ft) = p.strip_prefix("ft=").or_else(|| p.strip_prefix("filetype=")) {
    ...
    if text.contains("def ") && (text.contains("import ") || text.contains("class ") || text.contains("print(")) {
        return Some("py");
    }
...
impl LangConfig {
    fn from_path(path: &str) -> Option<Self> {
        ...
        let (language, name, hq, iq, lq): (tree_sitter::Language, &str, &str, &str, &str) = match ext.as_str() {
            "rs" => (
            ...
            _ => {
                ...
                return detected.and_then(|det_ext| {
                    Self::from_ext(det_ext)
                });
            }
        };
        ...
        Some(LangConfig { hl_config, lang_name: name })
    }
        ...
        match event {
            ...
            HighlightEvent::Source { start, end } => {
                if let Some(h) = current_hl {
                    ...
                    let s_line = line_starts.partition_point(|&ls| ls <= start).saturating_sub(1);
                    let e_line = line_starts.partition_point(|&ls| ls <= end.saturating_sub(1)).saturating_sub(1);
                    for line_idx in s_line..=e_line {
                        if line_idx >= result.len() { break; }
                        let ls = line_starts[line_idx];
                        let line_end = line_starts.get(line_idx + 1).map(|&e| e.saturating_sub(1)).unwrap_or(source.len());
...
struct Editor {
    ...
    theme: String,
    // Mouse text selection: (row, col) in document coords
...
impl Editor {
    fn open(path: &str) -> Self {
        ...
        let highlights = lang.as_mut().map(|l| compute_highlights(&source, l, &init_theme)).unwrap_or_default();
    ...
    fn current_line_len(&self) -> usize { self.line_char_len(self.cursor_row) }
    ...
    fn ensure_cursor_visible(&mut self) {
        ...
        let content_w = w.saturating_sub(GUTTER);
    ...
    fn push_undo(&mut self) {
        if self.undo_stack.last().map(|e: &UndoEntry| e.lines == self.lines).unwrap_or(false) {
            return;
        }
    ...
    fn selection_bounds(&self) -> Option<(usize, usize, usize, usize)> {
        ...
        if anchor.0 < end.0 || (anchor.0 == end.0 && anchor.1 <= end.1) {
            Some((anchor.0, anchor.1, end.0, end.1))
        } else {
            Some((end.0, end.1, anchor.0, anchor.1))
        }
    }
    ...
    fn check_external_modification(&mut self) {
        let current_mtime = std::fs::metadata(&self.file_path).ok().and_then(|m| m.modified().ok());
    ...
    fn draw(&mut self, frame: &mut Frame) {
        ...
        let sep_area     = Rect::new(size.x, size.y + content_h, size.width, 1);
        let status_area  = Rect::new(size.x, size.y + content_h + 1, size.width, 1);
        ...
        for row in visible_start..visible_end {
            gutter_lines.push(Line::from(Span::styled(
                format!("{:>4} ", row + 1),
                Style::default().fg(
                    if row == self.cursor_row { Color::Rgb(150, 150, 150) }
            ...
            let visible_chars: Vec<char> = line_chars.iter()
            ...
            let spans = if self.highlights.is_empty() || row >= self.highlights.len() {
                ...
                build_line_spans(&visible_chars, &self.highlights[row], self.scroll_col, sel_range)
            };
        ...
        // Cursor — only visible when within the current viewport
        let crow_s = self.cursor_row as isize - self.scroll_row as isize;
        let ccol_s = self.cursor_col as isize - self.scroll_col as isize;
        if crow_s >= 0 && crow_s < content_h as isize
            && ccol_s >= 0 && ccol_s < text_w as isize
        {
        ...
        if !self.embedded {
            ...
            if v_scrollable > 0 && vscroll_area.width > 0 && vscroll_area.height > 0 {
                ...
                for i in 0..track_h {
                    ...
                    if y >= vscroll_area.y + vscroll_area.height { break; }
        ...
        if !self.embedded && h_scrollable > 0 && sep_area.width > 0 {
            ...
            let thumb_size = ((track_w as f64 * track_w as f64)
                / (h_scrollable + track_w) as f64)
                .ceil()
                .max(1.0)
            ...
            for i in 0..track_w {
                ...
                if x >= sep_area.x + sep_area.width { break; }
                let is_thumb = i >= thumb_pos && i < thumb_pos + thumb_size;
                let (ch, fg) = if is_thumb {
                    ("▄", Color::Rgb(100, 100, 180))
                } else {
        ...
        let pos_str = if let Some((sr, sc, er, ec)) = self.selection_bounds() {
            ...
            if sel_lines > 1 {
                ...
                format!("Ln {}–{}  Col {}:{} ({} lines)", sr + 1, er + 1, sc + 1, ec + 1, sel_lines)
            } else {
        ...
        // Build right hint
        let right_str: &str = if self.override_confirm {
        ...
        let right_span = Span::styled(right_str, dim_gray);
...
fn build_line_spans<'a>(
    ...
    if let Some((sel_start, sel_end)) = sel_range {
        ...
        for i in vs..ve {
            bg_colors[i] = Some(Color::Rgb(55, 85, 150));
        }
    }
    ...
    match me.kind {
        MouseEventKind::Down(MouseButton::Left) => {
            ...
            if row < content_h {
                ...
                editor.sel_anchor = Some((target_row, target_col));
                editor.sel_end = None;
        ...
        MouseEventKind::Drag(MouseButton::Left) => {
            ...
            editor.sel_end = Some((target_row, target_col));
        }
        MouseEventKind::Up(MouseButton::Left) => {
            // Selection is finalised on Up — nothing extra needed
        }
...
fn handle_key(editor: &mut Editor, key: KeyEvent) -> Action {
    ...
    if !is_ctrl_q && !is_ctrl_x { editor.quit_confirm = false; }
    ...
    match (key.code, key.modifiers) {
        ...
        // Save (with external-modification guard)
        (KeyCode::Char('s'), m) if m.contains(KeyModifiers::CONTROL) => {
            if editor.external_modified && !editor.override_confirm {
        ...
        // Copy selection via OSC 52
        (KeyCode::Char('c'), m) if m.contains(KeyModifiers::CONTROL) => {
            if let Some((sr, sc, er, ec)) = editor.selection_bounds() {
                let mut text = String::new();
                for row in sr..=er {
                    if row >= editor.lines.len() { break; }
                    let line_chars: Vec<char> = editor.lines[row].chars().collect();
                    let start = if row == sr { sc } else { 0 };
                    let end   = if row == er { ec } else { line_chars.len() };
                    let segment: String = line_chars[start.min(line_chars.len())..end.min(line_chars.len())].iter().collect();
                    if row > sr { text.push('\n'); }
        ...
        (KeyCode::Backspace, m) if m.contains(KeyModifiers::CONTROL)
            || m.contains(KeyModifiers::ALT) => editor.delete_word_back(),
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        // Report scroll state to aide via OSC window title:
        // "aide:{scroll_row}/{total_lines}/{view_h}/{scroll_col}/{max_col}"
        {
            use std::io::Write as _;
            let mut out = io::stdout();
            let _ = write!(out, "\x1b]2;aide:{}/{}/{}/{}/{}\x07",
                editor.scroll_row,
                editor.line_count(),
                editor.inner_h,
                editor.scroll_col,
